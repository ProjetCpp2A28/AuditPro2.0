...
{
    QString idToExport = ui->lineEdit_export_emp->text();
    employe E;
...
void welcome::on_pushButton_export_clicked()
{
    ...
    if (!details.isEmpty()) {
        QString fileName = "entreprise_details_" + idToExport + ".pdf"; // Corrected this line
...
/*void welcome::update_label() {
    if (!codeExecuted) {
        ...
        QString firmwarePrefix = "Firmware Version: 0x92 = v2.0";
        QString nullBytes = QString(QByteArray(20, '\x00')); // Assuming you want to exclude 20 consecutive null bytes

        if (!data.trimmed().isEmpty() && data != firmwarePrefix && data != nullBytes) {
            ui->etat->setText(data);
            int id = data.toInt();

            if (E.checkid(id)) {
                qDebug() << "ok access";
                A.write_to_arduino2("3");
            } else {
                qDebug() << "pas d'acces";
                A.write_to_arduino2("2");
            }
...
void welcome::on_pushButton_export_rec_clicked()
{
    QString idToExport = ui->lineEdit_2->text();
    Reclamation REC;
    QString details = REC.getDetailsByID(idToExport);

    if (!details.isEmpty()) {
        QString fileName = "reclamation_details_" + idToExport + ".pdf";
        ...
        qDebug() << "Error: Reclamation not found or an error occurred.";
        QMessageBox::critical(this, QObject::tr("Export Error"),
                              QObject::tr("Reclamation not found or an error occurred."),
                              QMessageBox::Ok);
    }
